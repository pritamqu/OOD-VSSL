...
import math
import sys
from tools import AverageMeter, ProgressMeter, warmup_cosine_scheduler, warmup_multistep_scheduler, set_grad
...
def get_args():
    ...
    parser.add_argument('-c', '--config-file', type=str, help="pass config file name", 
                        default="byol_train3rd_test1st.yaml")
...
def finetune(args, cfg, backbone_state_dict, ngpus_per_node, logger, tb_writter, wandb_writter):
    ...
    ood_db = cfg['dataset']['ood_test']['name']+'_'+cfg['dataset']['ood_test']['subset']
    ...
    best_ood_model=os.path.join(args.ckpt_dir, f"best_model_{ood_db}.pth.tar")
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        if (epoch+1) % cfg['eval_freq'] == 0 or (epoch+1) == end_epoch:
            ...
            logger.add_line(f'IID Validation at Epoch: {epoch+1} meanAP: {vid_top1}')
            top1 = vid_top1
            if top1>=best_top1:
                best_top1=top1
                best_epoch=epoch
