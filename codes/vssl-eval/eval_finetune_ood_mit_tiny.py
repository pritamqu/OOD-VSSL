...
from tools import calculate_prec_recall_f1
...
def get_args():
    ...
    ## dataset and config
    parser.add_argument("--db", default="mitv2", help="target db", choices=['mitv2']) 
    parser.add_argument('-c', '--config-file', type=str, help="pass config file name", 
                        default="byol_mit_tiny_v2.yaml",
                        )
...
def finetune(args, cfg, backbone_state_dict, ngpus_per_node, logger, tb_writter, wandb_writter):
    ...
    ood_test_criterion = torch.nn.BCEWithLogitsLoss() # for multi-class
    ...
    ood_vid_top1, ood_vid_top5 = run_phase_multilabel(phase='test_dense', 
                                    loader=ood_test_loader, 
                                    model=model,
                                    optimizer=None, 
                                    criterion=ood_test_criterion, # criterion
                                    mixup_fn=None,
                                    fwd_kwargs=fwd_kwargs,
                                    lr_scheduler=None, 
                                    wd_scheduler=None, 
    ...
    if tb_writter is not None:
        tb_writter.add_scalar('fine_tune_epoch/ood_f1', ood_vid_top1, epoch)
        tb_writter.add_scalar('fine_tune_epoch/ood_acc', ood_vid_top5, epoch)
    if wandb_writter is not None:
        wandb_writter.log({'fine_tune_epoch/ood_f1': ood_vid_top1, 'custom_step': epoch})
        wandb_writter.log({'fine_tune_epoch/ood_acc': ood_vid_top5, 'custom_step': epoch})   
        
    # --------- end log       
    logger.add_line(f'InD Acc - top1: {best_top1} - top5: {best_top5}')
    logger.add_line(f'OOD F1: {ood_vid_top1} - Acc: {ood_vid_top5}')
...
def run_phase_multilabel(phase, loader, model, optimizer, criterion, mixup_fn, fwd_kwargs,
              lr_scheduler, wd_scheduler, amp,
              epoch, args, logger, tb_writter, wandb_writter, print_freq, ensemble='mean'):
    ...
    if True:
        ...
        # results = calculate_prec_recall_f1(pred_holder.cpu().numpy(), target_holder.cpu().numpy(), logger)
