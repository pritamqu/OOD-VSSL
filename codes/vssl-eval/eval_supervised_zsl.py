...
from tools import warmup_cosine_scheduler, warmup_multistep_scheduler
...
def get_args():
    ...
    ## dataset and config
    parser.add_argument("--db", default="kinetics400", help="target db", choices=['kinetics400'])  
    parser.add_argument('-c', '--config-file', type=str, help="pass config file name", 
                        default="superv_zsl.yaml")
...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    zero_shot_recognition(args, cfg, None, ngpus_per_node, logger=logger, tb_writter=tb_writter, wandb_writter=wandb_writter)
