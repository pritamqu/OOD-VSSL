...
class MLP(nn.Module):
    def __init__(self, in_dim, hidden_dim=1024, out_dim=768, nlayers=1, use_bn=False):
        ...
        layers.extend(
            nn.ModuleList([
                nn.Linear(hidden_dim, out_dim), # added in the main loop below.
            ])
        )
...
class DebiasHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.f1_conv3d = MLP(in_dim=head_in_dim, hidden_dim=head_hidden_dim, out_dim=num_classes, nlayers=2, use_bn=True)
        ...
        if bias_input:
            self.f2_conv3d = MLP(in_dim=head_in_dim, hidden_dim=head_hidden_dim, out_dim=num_classes, nlayers=2, use_bn=True)
            ...
            #     norm_cfg=dict(type='BN3d', requires_grad=True))
        if bias_network:
            self.f3_conv2d = MLP(in_dim=head_in_dim, hidden_dim=head_hidden_dim, out_dim=num_classes, nlayers=2, use_bn=True)
        ...
        if self.dropout_ratio != 0:
            ...
            self.dropout = None
        # self.f1_fc = nn.Linear(head_hidden_dim * 2, self.num_classes)
        # self.f2_fc = nn.Linear(head_hidden_dim * 2, self.num_classes)
        # self.f3_fc = nn.Linear(head_hidden_dim * 2, self.num_classes)
    ...
    # def forward_original(self, x, num_segs=None, target=None, **kwargs):
    ...
    def forward(self, feat, feat_shuffle, feat_reshape, num_segs=None, target=None, **kwargs):

        # mean of all the patches
        # x = feat
        y = torch.eye(self.num_classes).to(feat.device)
        y = y[target]
        losses = dict()
        ...
        # feat_unbias
        x = self.f1_conv3d(feat)  # (B, 2048, 8, 7, 7)
        # feat_unbias = x # self.avg_pool(x).squeeze(-1).squeeze(-1).squeeze(-1)
        # x = self.dropout(feat_unbias)
        # x = self.f1_fc(x)
        alpha_unbias = self.exp_evidence(x) + 1
        # minimize the edl losses
        ...
        # 
        # embed_size = feat.shape[2]
        # batch_size = feat.shape[0]
        # patch_size = feat.shape[1]
        # # FIXME: DO NOT HARD CODE 16; do somthing later       
        # temp_size = patch_size//49 # 112//16 -> 7
        # feat = feat.view(-1, temp_size, 7, 7, embed_size)
        # feat_shuffle = None # take as input
        if self.bias_input:
            # f2_Conv3D(x)
            # TODO: either shuffle the patches along time axis; or shuffle input
            # temporal shuffle
            # feat_shuffle = feat[:, torch.randperm(feat.size()[1]), ::]
            # feat_shuffle = feat_shuffle.view(batch_size, patch_size, embed_size).mean(dim=1)
            x = self.f2_conv3d(feat_shuffle)  # (B, P, E)
            # feat_bias1 = x # self.avg_pool(x).squeeze(-1).squeeze(-1).squeeze(-1)
            # x = self.dropout(feat_bias1)
            # x = self.f2_fc(x)
            alpha_bias1 = self.exp_evidence(x) + 1
            # minimize the edl losses
            ...
            losses.update({'loss_bias1_cls': loss_cls2})
            if self.alternative:
                # did not modify this part as set in false in paper
        ...
        # feat_reshape = None # take a 2d input
        if self.bias_network:
            # f3_Conv2D(x)
            # TODO: resume here
            # just accept input as 2D and temp shuffles and cal
            # B, C, T, H, W = feat.size()
            # feat_reshape = feat.permute(0, 2, 1, 3, 4).contiguous().view(-1, C, H, W)  # (B*T, C, H, W)
            x = self.f3_conv2d(feat_reshape)  # (64, 2048, 7, 7)
            # x = x.view(B, T, x.size(-3), x.size(-2), x.size(-1)).permute(0, 2, 1, 3, 4)  # (B, 2048, 8, 7, 7)
