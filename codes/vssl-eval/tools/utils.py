...
def sanity_check(args):
    ...
    if args.resume: # loading resumed config file
        _cfg_file = glob.glob(os.path.join(args.log_dir, '*', '*.yaml'))
        assert len(_cfg_file)==1, f"{_cfg_file}"
        assert os.path.isfile(_cfg_file[0]), f"{_cfg_file[0]}"
        args.cf = _cfg_file[0].split('/')[-1]
        args.config_file = _cfg_file[0]
