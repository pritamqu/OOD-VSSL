...
from optimizers import get_optimizer_mae, cosine_scheduler
...
def get_args(mode='default'):
    ...
    parser.add_argument('-c', '--config-file', type=str, help="config", default="dino.yaml")
...
def main(args):
    ...
    if args.debug:
        ...
        cfg['hyperparams']['temp_kwargs']['warmup_teacher_temp_epochs'] = 1
...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    # # define optimizer
    optimizer = get_optimizer_mae(name= cfg['hyperparams']['optimizer']['name'],
    ...
    temp_kwargs = cfg['hyperparams']['temp_kwargs']
    teacher_temp_schedule = np.concatenate((
            np.linspace(temp_kwargs['warmup_teacher_temp'], temp_kwargs['teacher_temp'], temp_kwargs['warmup_teacher_temp_epochs']),
            np.ones(cfg['hyperparams']['num_epochs'] - temp_kwargs['warmup_teacher_temp_epochs']) * temp_kwargs['teacher_temp']
        ))
    student_temp_schedule = np.ones(cfg['hyperparams']['num_epochs']) * temp_kwargs['student_temp']
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        if 'fwd_kwargs' in cfg['model']:
            ...
            fwd_kwargs['teacher_temp'] = teacher_temp_schedule[epoch]
            fwd_kwargs['student_temp'] = student_temp_schedule[epoch]
...
def train_one_epoch(args, cfg, model, optimizer, 
    ...
    for i, sample in enumerate(train_loader):
        ...
        data_dict.update({'teacher_temp':fwd_kwargs['teacher_temp']})
        data_dict.update({'student_temp':fwd_kwargs['student_temp']})
