...
def get_args(mode='default'):
    ...
    parser.add_argument('-c', '--config-file', type=str, help="config", default="simclr.yaml")
    ...
    for i, sample in enumerate(train_loader):
        ...
        if amp is not None: # mixed precision
            with torch.cuda.amp.autocast():
                if train_loader.dataset.return_video:
                    data_dict = model.forward(frames1, frames2, **fwd_kwargs)
        ...
        if ema_scheduler is not None:
            m = ema_scheduler[step] # video
            data_dict.update({'ema': m})
            model.module.update_target(m)
