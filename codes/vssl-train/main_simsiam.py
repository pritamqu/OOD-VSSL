...
def get_args(mode='default'):
    ...
    parser.add_argument('-c', '--config-file', type=str, help="config", default="simsiam.yaml")
...
def train_one_epoch(args, cfg, model, optimizer, 
    ...
    if fwd_kwargs is None:
        fwd_kwargs = {}
    clip_grad = fwd_kwargs.pop('clip_grad') if 'clip_grad' in fwd_kwargs else None
