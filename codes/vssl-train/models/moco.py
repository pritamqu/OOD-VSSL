...
class VideoMOCOv3(nn.Module): 
    ...
    def forward_loss(self, q, k, distributed, temperature):
        # normalize
        q = nn.functional.normalize(q, dim=1)
        k = nn.functional.normalize(k, dim=1)
        # gather all targets
        if distributed:
            k = dist_utils.AllGatherWithGradient.apply(k)
        # Einstein sum is more intuitive
        logits = torch.einsum('nc,mc->nm', [q, k]) / temperature
        N = logits.shape[0]  # batch size per GPU
        labels = (torch.arange(N, dtype=torch.long) + N * dist_utils.get_rank()).cuda()
        return nn.CrossEntropyLoss()(logits, labels) * (2 * temperature)
        
    def forward(self, x1, x2, mask_ratio, m, distributed, temperature, **kwargs):
        if mask_ratio==0:
            mask_ratio=None
        ...
        with torch.no_grad():
            ...
            k1 = self.target_encoder(self.prepare_token(x1))
            k2 = self.target_encoder(self.prepare_token(x2))
        
        loss = self.forward_loss(q1, k2, distributed, temperature) / 2 + self.forward_loss(q2, k1, distributed, temperature) / 2
...
if __name__=='__main__':
    ...
    model = VideoMOCOv3(frame_size = frame_size,
                    num_frames = num_frames,
    ...
    frames = torch.randn(2, 3, num_frames, frame_size[-1], frame_size[-1]).cuda()
    
    op = model(frames, frames, mask_ratio=0.5, m=0.996, distributed=False, temperature=0.1
            )
